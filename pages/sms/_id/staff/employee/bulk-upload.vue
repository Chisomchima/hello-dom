...
export default {
  ...
  methods: {
    ...
    mapStuff() {
      arr1
        .map((itm, a) => ({ ...itm, ...arr2.find((item) => a === item.index) }))
        .map(({ index, ...rest }) => {
          return rest;
        });
    },
    async uploadStaffs() {
      try {
        const { data } = await this.$axios.post(
          `/slate-admin/school/${this.schoolId}/staff/hr/bulk/`,
          {
        ...
        setTimeout(() => {
          this.$router.push("/sms/SVAA/staff/employee");
        }, 2000);
        ...
        if (error.response) {
          console.log("data coming in", this.staffData);
          ...
          this.staffData = this.staffData
            ...
            .map((itm, a) => ({
              ...itm,
              ...this.staffError.find((item) => a === item.index), // match error message to staff
            }))
            .map(({ staff_name, index, ...rest }) => {
              return rest; // remove unnecesary data
            })
            .filter((e) => {
              return e.error_message; // filter out staffs with no error message
            });
          this.showRecordbtn = true;

          console.log("new array after error is", this.staffData);
        }
      }
    },
    onChange(e) {
    ...
    downloadSample() {
      ...
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, data, "data");
      XLSX.writeFile(wb, "Sample_Staff.xlsx");
    },
  },
};
</script>
