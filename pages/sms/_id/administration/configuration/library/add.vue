                  ...
                  <div class="col-8 col-lg-9 col-xl-10">
                    ...
                    <div class="mt-2 form-group">
                      ...
                      <!-- <input
                        v-model="form.author"
                        type="text"
                        placeholder="Seperate authors by comma eg Adamu , Ada JohnSon"
                        class="form-control"
                      /> -->
                      <vue-tags-input
                        v-model="form.author"
                      ...
                      >
                        ...
                        <div slot="tag" slot-scope="props"></div>
                      </vue-tags-input>
                      <!-- <v-select
                        ref="multiple"
                        v-model="form.author"
  ...
  methods: {
    ...
    async submitForm() {
      ...
      try {
        submit.forEach((e) => {
          console.log('tags')
          // console.log(JSON.stringify(e.author).replace('[', ''))
          console.log('for submit', e)
          e.author = this.tags.map((e) => e.text).join(',')
          e.subjects = e.subjects.join(',')
          e.grades = e.grades.join(',')
          e.publishers = e.publishers.join(',')
        })
        await this.$axios.post(`/schools/v2/school/${this.schoolId}/library/`, {
          data: submit,
        })

        this.$router.push(
          `/sms/${this.schoolCode}/administration/configuration/library`
        )
      } catch (error) {
        console.error(error)
      }
