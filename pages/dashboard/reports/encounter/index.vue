        ...
        >
          <template v-if="items.length > 0" #besideFilterButton>
            <div class="d-flex align-items-center">
              ...
              <button @click="downloadEncReport" class="btn btn-sm btn-primary">
          ...
          >
            <template #status="{ data }">
              <div v-if="data.item.status === 'NS'" class="text-capitalize">Nurse seen</div>
              <div v-if="data.item.status === 'DS'" class="text-capitalize">Closed</div>
              <div v-if="data.item.status === 'New'" class="text-capitalize">{{ data.item.status }}</div>
            </template>
          </TableComponent>
        </UtilsFilterComponent>
      </div>
