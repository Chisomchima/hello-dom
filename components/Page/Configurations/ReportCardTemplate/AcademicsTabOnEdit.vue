                    ...
                    v-model="fieldObj.element[index]"
                     class="form-control">
                      <template v-for="(entry, index2) in items">
                        <option
                          :key="index2"
                          :value="{name:entry.name, column_id:entry.column_id}"
                        >{{entry.name}}</option>
                      </template>
                    </select>
                  </div>
...
export default {
  ...
  methods: {
    handleOk(e) {
      ...
      if (this.onEdit) {
        if (this.fieldObj.type === 'total') {
          this.fieldObj.element = this.fieldObj.element.map((item) => item.column_id)
        }
    ...
    editItem(e) {
      ...
      if (this.fieldObj.type === 'total') {
        this.fieldObj.element = this.fieldObj.element.map((item) => {
          const columnObj = this.items.find(
            (column) => column.column_id === item
          )
          return {
            name: columnObj.name,
            column_id: columnObj.column_id,
          }
        })
      }
    ...
    // updateSelectedElement(e,index) {
    //   this.fieldObj.element[index] = {name: e.name, id: e.column_id}
    // },
  },
}
</script>
