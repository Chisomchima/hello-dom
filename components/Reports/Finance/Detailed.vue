    ...
    <div>
        <div class="row">
            <div class="col-md-12 mb-4">
                <div class="card">
                    <div class="card-body">
                        <ReportsFinanceDetailedFilters @filter="filter(1, $event)" />
                    </div>
                </div>
            </div>
            <div class="col-md-12">
...
export default {
    ...
    methods: {
        async filter(page, e) {
            ...
            try {
                this.busy = true
                const data = await this.$api.reports.financeDetailedReport({ ...e, page })
        ...
        async downloadFinanceReport() {
            if (this.items.length > 0) {
                ...
                if (response.status === 200) {
                    ...
                    link.download = `Finance Detailed Report - ${this.date}`
                    link.href = objectURL
