    ...
    <div>
      <div class="row">
        <div class="col-md-12 mb-4">
          <div class="card">
            <div class="card-body">
              <ReportsPaymentsSummaryFilter @filter="filter(1, $event)" />
            </div>
          </div>
        </div>
        <div class="col-md-12">
  ...
  export default {
    ...
    methods: {
      async filter(page, e) {
        ...
        try {
          this.busy = true
          const data = await this.$api.reports.paymentsSummaryReport({ ...e, page })
          this.items = data
      ...
      async downloadFinanceReport() {
        ...
        //   }
          ...
          const response = await fetch(
            `${process.env.BASE_URL}finance/reports/payments/summary?${download_string}`,
            {
              headers: {
          ...
          if (response.status === 200) {
            ...
            link.download = `Payments Summary Report - ${this.date}`
            link.href = objectURL
