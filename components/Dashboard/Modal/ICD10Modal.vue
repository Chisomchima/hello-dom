<template>
  <div>
    <ModalWrapper size="xl" id="icd10modal" :cancelText="tray.length > 0 ? 'Select' : 'close'" :arrangement="false" @hide="addDiagnosis" :title="`Diagnosis lists`">
      <div>
        <div class="d-flex flex-wrap text-white">
          <div v-for="(item, index) in tray" :key="index" :class="item.confirmed ? 'bg-success' : 'bg-info'" class="
              d-flex
        ...
        <div class="p-3">
          <input type="text" placeholder=" Type to search..." class="form-control ng-untouched ng-pristine ng-valid"
            v-model="searchParam" />
        </div>
        <TableComponent :items="dataOBJS" :fields="fields" :pages="pages" :busy="busy" @page-changed="goToNextPage">
          <template #clear="{ data }">
            <label class="exercise-option-check blue-check">
              <input type="checkbox" name="customRadio" @change="pick($event.target.checked, data.item, data.index)" />
              <span class="checkmark"></span>
            </label>
          </template>
          <template #dial="{ data }">
            <div class="d-flex align-items-center">
              <b-form-checkbox size="lg" switch @change="setToConfirmed(data.item, $event)">
              </b-form-checkbox>
              <div v-if="data.item.confirmed == true" class="mt-2 text-success text-12">
                Confirmed
              </div>
...
export default Vue.extend({
  ...
  methods: {
    ...
    async addDiagnosis() {
      let arr = this.tray
      this.$emit('incomingDiagnosis', arr)
      // this.tray = []
