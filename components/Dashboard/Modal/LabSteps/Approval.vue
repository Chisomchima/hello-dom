<template>
    <div>
        <b-modal size="lg" id="Edit-laborder" title="Approve Result" centered hide-footer @hide="cancelPanelOrder">
                ...
                ">
                            <TableComponent :items="labOrderPanel.panel.obv" :fields="fields" :paginate="false">
                                <template #value="{ data }">
                                    ...
                                    <div class="col-12 px-0" v-if="data.item.type.name === 'Options'">
                                        <ValidationProviderWrapper :rules="[]">
                                            <v-select class=" text-14 w-100" placeholder="Select option" label="name"
                                                v-model="data.item.value" :options="data.item.values"
                                                :disabled="enabled" :reduce="(option) => option.name"></v-select>
                                        </ValidationProviderWrapper>
                                    </div>
                                </template>
        ...
        " style="height: 38px; width: 5rem; text-align: center">
                                ...
                                <div v-if="approveControl" class="d-flex justify-content-between">
                                    <button @click="enableButton" class="btn btn-light text-grey mx-3 text-14"
                                        style="height: 38px; width: 5rem; text-align: center">
                                        Edit
                                    </button>

                                    <button @click="saveAndApprove" class="
                                btn
                                text-white text-14
                                mainbtndashboard
                                medbrownparagraph
...
" style="height: 38px; text-align: center">
                                    ...
                                    <button @click.prevent="setStatusToReject" class="
                            btn btn-danger
                            text-white text-14
                            mainbtndashboard
                            medbrownparagraph
                            ...
                            " style="height: 38px; width: 5rem; text-align: center">
                                ...
                                <div v-if="editMode" class="d-flex justify-content-between">
                                    <button @click.prevent="disabledButton" class="btn btn-light text-grey mx-3 text-14"
                                        style="height: 38px; width: 5rem; text-align: center">
                                        Cancel
                                    </button>
