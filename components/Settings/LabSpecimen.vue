<template>
    <div>
        <div>
            <b-modal @hide="closeModal" id="Add-specimen" title="Add Specimen Type" centered hide-footer
                ...
                ">
                                <div class="mb-2 col-lg-10 px-0 col-md-10 col-sm-10">
                                    ...
                                    <validation-provider rules="required" v-slot="{ errors }">
                                        <input type="text" placeholder="Name" v-model="specimen.name"
                                ...
                                <div class="mb-2 w-75">
                                    ...
                                    <div style="border-radius: 4px; background: #5c677d" class="
                      d-flex
                      p-3
                      ...
                      w-100
                    ">
                                        <div :class="ring1 ? 'ring' : ''" style="width: 2rem; border-radius: 50%"
                                            @click.prevent="setPurple" class="first back-purple pointer"></div>
                                        <div :class="ring2 ? 'ring' : ''" @click.prevent="setYellow"
                                            style="width: 2rem; border-radius: 50%" class="second back-yellow pointer">
                                        </div>
                                        <div :class="ring3 ? 'ring' : ''" @click.prevent="setGrey"
                                            style="width: 2rem; border-radius: 50%" class="third back-grey pointer">
                                        </div>
                                        <div :class="ring4 ? 'ring' : ''" @click.prevent="setWhite"
                                            style="width: 2rem; height: 2rem; border-radius: 50%"
                                            class="fourth back-white pointer"></div>
                                        <div :class="ring5 ? 'ring' : ''" @click.prevent="setRed"
                                            style="width: 2rem; border-radius: 50%" class="fifth back-red pointer">
                                        </div>
                                        <div :class="ring6 ? 'ring' : ''" @click.prevent="setGreen"
                                            style="width: 2rem; border-radius: 50%" class="fifth back-green pointer">
                                        </div>
                                        <div :class="ring7 ? 'ring' : ''" @click.prevent="setBlue"
                                            style="width: 2rem; border-radius: 50%" class="fifth back-blue pointer">
                                        </div>
                                    </div>

                                    <input type="text" v-model="specimen.color" class="
                      form-control
                      d-none
                    ...
                    " />
                                ...
                                <div class="mb-2 col-lg-10 px-0 col-md-10 col-sm-10">
                                    ...
                                    <validation-provider rules="required" v-slot="{ errors }">
                                        <input type="text" v-model="specimen.description" placeholder="description"
                                            class="form-control ng-untouched ng-pristine ng-valid" />
                                        <span class="text-12" style="color: red">{{
                                            errors[0]
                                            }}</span>
                            ...
                            <div class="my-3 d-flex justify-content-center">
                                ...
                                <BaseButton @click.prevent="addSpecimenType" class="">Save
                                </BaseButton>
                            </div>
        ...
        <div>
            <UtilsFilterComponent disable-pagination :disable-visualization="true" search-placeholder="Search">
                <template #besideFilterButton>
                    <BaseButton class="btn-outline-primary" @click="openLabObservationsModal">Add specimen type
                    </BaseButton>
                </template>
                <table-component :paginate="true" :fields="fields" :perPage="perPage" :busy="busy" :pages="pages"
                    :items="specimens" @page-changed="getSpecimens($event)">
                    <template #status="{ data }">
                        <div class="text-center">
                            <span class="badge border border-danger text-white badge-info px-2 py-1">{{ data.item.status
                                }}</span>
                        </div>
                    </template>
                    <template #color="{ data }">
            ...
            " :style="`background: ${data.item.color}`" class="first pointer"></div>
                    </template>
                    <template #edit="{ data }">
        ...
        <b-modal @hide="closeEditModal" id="Edit-specimen" title="Edit Laboratory Observations" centered hide-footer
            ...
            <div>
              ...
              ">
                            <div class="mb-2 col-lg-10 px-0 col-md-10 col-sm-10">
                                <small class="text-grey text-12">Name *</small>
                                <validation-provider rules="required" v-slot="{ errors }">
                                    <input v-model="editSpecimen.name" type="text" placeholder="Name"
                                        class="form-control ng-untouched ng-pristine ng-valid" />
                                    <span class="text-12" style="color: red">{{
                                        errors[0]
                                        }}</span>
                            ...
                            <div class="mb-2 col-lg-10 px-0 col-md-10 col-sm-10">
                                ...
                                <div style="border-radius: 4px; background: #5c677d"
                                    class="d-flex mx-4 p-3 justify-content-between border">
                                    <div :class="ring1 ? 'ring' : ''" style="width: 2rem; border-radius: 50%"
                                        @click.prevent="setPurpleEdit" class="first back-purple pointer"></div>
                                    <div :class="ring2 ? 'ring' : ''" @click.prevent="setYellowEdit"
                                        style="width: 2rem; border-radius: 50%" class="second back-yellow pointer">
                                    </div>
                                    <div :class="ring3 ? 'ring' : ''" @click.prevent="setGreyEdit"
                                        style="width: 2rem; border-radius: 50%" class="third back-grey pointer"></div>
                                    <div :class="ring4 ? 'ring' : ''" @click.prevent="setWhiteEdit"
                                        style="width: 2rem; height: 2rem; border-radius: 50%"
                                        class="fourth back-white pointer"></div>
                                    <div :class="ring5 ? 'ring' : ''" @click.prevent="setRedEdit"
                                        style="width: 2rem; border-radius: 50%" class="fifth back-red pointer"></div>
                                    <div :class="ring6 ? 'ring' : ''" @click.prevent="setGreenEdit"
                                        style="width: 2rem; border-radius: 50%" class="fifth back-green pointer"></div>
                                    <div :class="ring7 ? 'ring' : ''" @click.prevent="setBlueEdit"
                                        style="width: 2rem; border-radius: 50%" class="fifth back-blue pointer"></div>
                                </div>

                                <input type="text" v-model="editSpecimen.color"
                                    class="form-control d-none ng-untouched ng-pristine ng-valid" />
                            </div>
                            <div class="mb-2 col-lg-10 px-0 col-md-10 col-sm-10">
                                <small class="text-grey text-12">Description</small>
                                <validation-provider rules="required" v-slot="{ errors }">
                                    <input v-model="editSpecimen.description" type="text" placeholder="uom"
                                        class="form-control ng-untouched ng-pristine ng-valid" />
                                    <span class="text-12" style="color: red">{{
                                        errors[0]
                                        }}</span>
                        ...
                        <div class="my-3 d-flex justify-content-center">
                            ...
                            <BaseButton @click.prevent="editSpecimenfunc" class="">Save
                            </BaseButton>
                        </div>
...
export default {
    ...
    methods: {
        ...
        async getSpecimens(page = 1) {
            try {
                this.busy = true;
                let uri = `laboratory/lab_specimen_type/?page=${page}&size=${this.perPage}`;
