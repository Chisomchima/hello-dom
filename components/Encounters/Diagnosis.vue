      ...
      <div v-show="tag" class="mr-3">
        ...
        <div class="row">
          <div class="col-lg-8 col-md-6 col-sm-6">
            <small class="text-grey text-14">Diagnosis</small><input type="text" placeholder="Type here..."
              v-model="diagnosis.comment" class="form-control ng-untouched ng-pristine ng-valid" />
          </div>
          <div class="col-lg-3 col-md-4 col-sm-6 w-100">
            <small class="text-grey text-14">Options</small>
            <v-select class="text-14 text-capitalize" v-model="diagnosis.option" style="height: 35px;"
      ...
      <div class="pt-3" v-if="comments.length > 0">
        <TableComponent  :items="comments" :perPage="1000" :fields="fields" :paginate="false">
          ...
          <template #option="{ data }">
            <div>
              <div class="p-2" v-if="data.item.value.option === 'working' || data.item.value.option === 'confirmed'">
                <v-select class="text-capitalize text-14" placeholder="Select option" label="name" v-model="data.item.value.option"
                  :options="options" :disabled="true" ></v-select>
              </div>
              <div class="p-2" v-else-if="data.item.value.option === ''">
                <v-select class="text-capitalize text-14" placeholder="Select option" label="name" v-model="diagnosis.option"
                  :options="options" ></v-select>
              </div>
              <div class="p-2" v-else>
                <v-select class="text-capitalize text-14" placeholder="Select option" label="name" 
                  :options="options" ></v-select>
              </div>
            </div>
          </template>
          <template #action="{ data }">
            <!-- <pre>{{data}}</pre> -->
            <span @click="confirmDiagnosis(data.item.value)" v-if="data.item.value.option === 'working'"
            ...
            <div v-else-if="data.item.value.option === ''" class="text-14 text-start p-1 text-white">
             <span @click="addDiagnosis"  class="text-success m-1"><svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="img" width="20" height="20" preserveAspectRatio="xMidYMid meet" viewBox="0 0 1024 1024"><path fill="currentColor" d="M912 190h-69.9c-9.8 0-19.1 4.5-25.1 12.2L404.7 724.5L207 474a32 32 0 0 0-25.1-12.2H112c-6.7 0-10.4 7.7-6.3 12.9l273.9 347c12.8 16.2 37.4 16.2 50.3 0l488.4-618.9c4.1-5.1.4-12.8-6.3-12.8z"/></svg></span>
             <span @click="deleteItem(data.item)" class="text-danger m-1">
              <svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="img" width="20" height="20" preserveAspectRatio="xMidYMid meet" viewBox="0 0 24 24"><path fill="currentColor" d="M6 7H5v13a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V7H6zm4 12H8v-9h2v9zm6 0h-2v-9h2v9zm.618-15L15 2H9L7.382 4H3v2h18V4z"/></svg>
             </span>
            </div>
          </template>
        </TableComponent>
